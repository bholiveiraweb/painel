...
class Product
{
    ...
    public function getProductFromId(int $id)
    {
        $stmt = $this->db->prepare("SELECT tb_products.*, tb_departments.title as department_title FROM tb_products 
                                INNER JOIN tb_departments ON tb_departments.id = tb_products.id_department WHERE tb_products.id = ?");
    ...
    {
        $stmt = $this->db->prepare("SELECT tb_products.*, tb_departments.title as department_title FROM tb_products INNER JOIN tb_departments ON tb_departments.id = tb_products.id_department WHERE tb_departments.slug = ?", [\PDO::ATTR_CURSOR => \PDO::CURSOR_FWDONLY]);
    ...
    {
        $stmt = $this->db->prepare("UPDATE tb_products SET id_department = :id_department, title = :title, description = :description, price = :price, link = :link, slug = :slug, updated_at = NOW() WHERE id = :id");
